...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        $mode === 'update'
        && $_REQUEST['addon'] === 'epayco'
        && (
            !empty($_REQUEST['pp_settings'])
            || !empty($_REQUEST['epayco_logo_image_data'])
        )
    ) {
        ...
        fn_update_epayco_settings($pp_settings, $storefront_id);
    }
...
if ($mode === 'update') {
    if ($_REQUEST['addon'] === 'epayco') {
        Tygh::$app['view']->assign('pp_settings', fn_get_epayco_settings($storefront_id));
    }
}
