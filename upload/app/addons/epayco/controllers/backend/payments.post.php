...
use Tygh\Enum\Addons\Epayco\Processors;
use Tygh\Registry;
...
if ($mode === 'processor') {
    ...
    $is_epayco_processor = false;
    if ($processor_id !== null) {
        $is_epayco_processor = fn_is_epayco_processor($processor_id);
    }

    if ($is_epayco_processor) {
        ...
        $epayco_currencies = fn_epayco_get_currencies($type);

        /** @var \Tygh\SmartyEngine\Core $view */
        $view = Tygh::$app['view'];

        $view->assign('epayco_currencies', $epayco_currencies);
    }
}
